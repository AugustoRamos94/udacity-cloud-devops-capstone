        ...
        stage('Create Kubernetes Config'){
            steps {
                withAWS(region:'us-west-2',credentials:'aws') {
                    sh '''
                         eksctl create cluster --name kubeClusters --version 1.15 --region us-west-2 --nodegroup-name node-groups --node-type t2.small --nodes 2 --nodes-min 1 --nodes-max 4 --managed
                         '''
                }
            }
        }
            ...
            steps {
                 withAWS(credentials:'aws') {
                     sh '''
                         ...
                         kubectl apply -f flask-container.yml
